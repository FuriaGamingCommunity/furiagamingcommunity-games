	...
	class FuriaGamingCommunity_Games_BP_Group_Extension extends BP_Group_Extension {
		...
		function settings_screen( $group_id = NULL ) {
			...
			<div>
				...
				<p><?php _e( 'Game group types are used to define different <strong><em>group roles</em></strong>. Groups that are dedicated to the same game may have different purpouses, thus hey may have a different group type set.', 'furiagamingcommunity_games' ); ?></p>
			</div>
		...
		function settings_screen_save( $group_id = NULL ) {
			...
			if ( !empty( $_POST['group-game'] ) ) {
				$setting['group-game'] = $_POST['group-game'];

				$settings_saved = true;
			}

			if ( !empty( $_POST['group-game-type'] ) ) { 
				$setting['group-game-type'] = $_POST['group-game-type'];
				...
				if ( is_wp_error( $term_taxonomy_ids ) ) {
					bp_core_add_message( sprintf( __( 'An error occurred while adding the selected game group type: %1$s',  'furiagamingcommunity_games' ), $message ), 'error' );
				} else {
					$settings_saved = true;
				}
			}

			if ( $settings_saved ) bp_core_add_message( __( '<srong>Game</strong> settings updated successfully.',  'furiagamingcommunity_games' ) );
